...
## 2026-09-13: ゼロ値が意味を持つフィールドはポインタにする

### 背景

`marshalEmbeddedInstance` はゼロ値フィールドを送信しない。これは
`ModifySystemSettings` の「最小インスタンス」パターン(全体を送り返すと read-only
プロパティでジョブが Exception になる)を成立させるために必要な挙動で、設計としては正しい。

しかし副作用として **`false` / `0` / `""` への変更を表現する手段が無かった**。
「変更しない」と「ゼロ値に変える」が区別できず、後者は常に黙殺される。

実機で確認した典型 (2026-09-09):

```
AutomaticSnapshotsEnabled=true  を送る → 反映される
AutomaticSnapshotsEnabled=false を送る → 黙殺 (true のまま)
```

MOF 上 Read/write なので Hyper-V が拒否しているのではなく、**こちらが送っていなかった**。
ホスト既定が `true` / 利用側の既定が `false` というフィールドでは、新規 VM のたびに踏む。

terraform-provider-hyperv 側は「ゼロ値ダウングレードを検知して PowerShell へ委譲する」
ガードで回避していたが、それが 3 箇所に増えた時点で対症療法の限界だった。

### 検討した案

| 案 | 内容 | 判断 |
|---|---|---|
| **A: ポインタフィールド** | ゼロ値が意味を持つフィールドだけ `*bool` 等にする | **採用** |
| B: 構造体タグのオプション | `cim:"X,sendzero"` で「常に送る」 | 却下。そのフィールドを触らない更新でも送ってしまい、最小インスタンスの原則が崩れる |
| C: 送信フィールドの明示リスト | `WithFields("X")` で呼び出し側が指定 | 却下。フィールド名の文字列指定になりリファクタで壊れる |
...
**必要なフィールドだけ順次移す。** 全フィールドをポインタにはしない
(165 個の大半はゼロ値に意味が無く、ポインタ化は可読性を下げるだけ)。
移す基準は **「ホスト既定が非ゼロ値 かつ 利用側がゼロ値を要求しうる」**。

read 側は「応答にプロパティが無ければ `nil` のまま」にする。
**「未指定」と「false」を区別できることがポインタ化の目的**なので、
読むたびに確保すると意味が無くなる。

### 検証

実機で `UpdateVm` (ModifySystemSettings) と `DefineSystem` の両経路で
`&false` が反映されることを確認した。経路ごとに受理するプロパティ集合が違うため、
片方だけの確認では不十分。

### 残る注意

- 純関数 (`marshalEmbeddedInstance`) のテストだけでは、呼び出し側がポインタを
  握り潰す変異を検出できない。`UpdateVm` / `DefineSystem` の配線テストが要る
- ポインタ化は **provider 側のコンパイルを壊す**。go.mod bump と provider 修正はセットで行う
